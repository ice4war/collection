...
export function HorizontalBar(input, id = "hbar") {
	...
	function chart() {
		...
		graph.append('g')
			...
			.call(d3.axisBottom(x).ticks(height / 40).tickSizeOuter(0).ticks(width / 40))
		// x axis End
		...
		const yTicks = d3.axisLeft(y).tickSizeOuter(0).ticks(height / 40);
		...
		const labels = g.append('g')
			...
			.call(t => t.filter(d => x(d.frequency) - x(0) < 22)
				.attr('dx', 4)
		...
		function zoom(graph) {
			...
			function zoomed(event) {
				...
				g.selectAll(`.${id}-bar`).attr('y', d => y(d.letter)).attr('height', y.bandwidth())
				g.selectAll(`.${id}-label`).attr('y', d => y(d.letter))
				graph.select('#y-axis').call(yTicks);
				labels.attr('x', d => x(d.frequency))
					.attr('y', d => y(d.letter) + y.bandwidth() / 2)
					.call(t => t.filter(d => x(d.frequency) - x(0) < 22)
						.attr('dx', 4)
						.attr('text-anchor', 'start')
