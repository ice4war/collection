...
export function normalizedStackHBar(input, id = "norstackhbar") {
	let width = 600;
	let height = 400;
	const margin = {
		"bottom": 50,
	...
	function chart() {
		...
		const y = d3.scaleBand()
			.domain(d3.groupSort(input, g => g.find(d => d.age === '<10').population / d3.sum(g, d => d.population), d => d.state))
			.range([height, 0])
			.padding(.1)
		...
		const legendG = graph.append('g').attr('id', 'legend')
			.attr('transform', `translate(${width/1.5},${-margin.top/3})`)


		const legendX = d3.scaleBand()
			.domain(color.domain())
			.range([0, width / margin.left * 12]).padding(.1)
		...
		legendG.append('g')
			...
			.call(g => g.selectAll('.tick text').attr('font-size', Math.floor(width % legendX.bandwidth() / 1.6)))
